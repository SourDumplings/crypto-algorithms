...
#define KE_ROTAES_WORD(x) (((x) << 8) | ((x) >> 24))
...
void ccm_prepare_first_ctr_blk(
	BYTE	   counter[],
	const BYTE nonce[],
	int		   nonce_len,
	int		   payload_len_store_size
);
void ccm_prepare_first_format_blk(
	BYTE	   buf[],
	int		   assoc_len,
	int		   payload_len,
	int		   payload_len_store_size,
...
)
{
	...
	if (mac_len != 4 && mac_len != 6 && mac_len != 8 && mac_len != 10 && mac_len != 12 &&
		mac_len != 14 && mac_len != 16)
		return (FALSE);
...
int aes_decrypt_ccm(
	...
	AES_WORD	   ciphertext_len,
	const BYTE	   assoc[],
	unsigned short assoc_len,
	const BYTE	   nonce[],
	unsigned short nonce_len,
	BYTE		   plaintext[],
	AES_WORD	  *plaintext_len,
	AES_WORD	   mac_len,
	int			  *mac_auth,
	const BYTE	   key_str[],
	int			   keysize
)
{
	...
	buf = (BYTE *)malloc(
		assoc_len + ciphertext_len /*ciphertext_len = plaintext_len + mac_len*/ + 48
	);
	if (!buf) return (FALSE);
	...
	*plaintext_len			 = ciphertext_len - mac_len;
	...
	increment_iv(
		temp_iv,
		AES_BLOCK_SIZE - 1 - mac_len
	);	// (AES_BLOCK_SIZE - 1 - mac_len) is the byte size of the counting portion of the counter
		// block.
	...
	if (mac_auth != NULL)
	{
		...
		// Perform the CBC operation with an IV of zeros on the formatted buffer to calculate the
		// MAC.
		memset(temp_iv, 0, AES_BLOCK_SIZE);
		aes_encrypt_cbc_mac(buf, end_of_buf, mac_buf, key, keysize, temp_iv);

		// Compare the calculated MAC against the MAC embedded in the ciphertext to see if they are
		// the same.
		if (!memcmp(mac, mac_buf, mac_len)) { *mac_auth = TRUE; }
		else
		{
			*mac_auth = FALSE;
...
AES_WORD SubWord(AES_WORD word)
{
	unsigned int result;
...
void aes_key_setup(const BYTE key[], AES_WORD w[], int keysize)
{
	int		 Nb = 4, Nr, Nk, idx;
	AES_WORD temp,
		Rcon[] = {
	...
	for (idx = Nk; idx < Nb * (Nr + 1); ++idx)
	{
		temp = w[idx - 1];
		if ((idx % Nk) == 0)
			temp = SubWord(KE_ROTAES_WORD(temp)) ^ Rcon[(idx - 1) / Nk];
...
void AddRoundKey(BYTE state[][4], const AES_WORD w[])
{
	BYTE subkey[4];
...
void aes_encrypt(const BYTE in[], BYTE out[], const AES_WORD key[], int keysize)
{
	BYTE state[4][4];
...
void aes_decrypt(const BYTE in[], BYTE out[], const AES_WORD key[], int keysize)
{
	BYTE state[4][4];
...
void print_rnd_key(AES_WORD key[])
{
	int idx;
