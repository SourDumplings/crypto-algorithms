...
void aes_key_setup(
	...
	int		   keysize
);	// Bit length of the key, 128, 192, or 256
...
int aes_encrypt_cbc(
	...
	const BYTE	   iv[]
);	// IV, must be AES_BLOCK_SIZE bytes long
...
void increment_iv(
	...
	int	 counter_size
);	// Bytes of the IV used for counting (low end)
...
int aes_encrypt_ccm(
	...
	int		   keysize
);	// IN  - The length of the key in bits. Valid values are 128, 192, 256.
...
int aes_decrypt_ccm(
	...
	unsigned short nonce_len,  // IN  - Nonce length in bytes.
	BYTE plaintext[],  // OUT - The plaintext that was decrypted. Will need to be large enough to
					   // hold ciphertext_len - mac_len.
	AES_WORD *plaintext_len,  // OUT - Length in bytes of the output plaintext, always
							  // ciphertext_len - mac_len .
	AES_WORD mac_len,		  // IN  - The length of the MAC that was calculated.
	int		*mac_auth,	// OUT - TRUE if authentication succeeded, FALSE if it did not. NULL pointer
						// will ignore the authentication.
